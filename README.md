...
The current app discovers injected wallets, verifies or permissionlessly deploys the predetermined protocol, publishes posts and comments with optional canonical IPFS CID commitments, reconstructs a confirmed global feed through bounded wallet-RPC reads, writes explicit like, unlike, and repost events, and derives exact confirmed reaction totals and visible comment histories through user-stepped RPC and local-cache work. Its protocol layer also supports verified complete profile-snapshot writes and strict event decoding; profile editing and projection UI are staged separately. Feed behavior and its deliberate work budget are documented in [`docs/feed.md`](./docs/feed.md); profile snapshots are documented in [`docs/profiles.md`](./docs/profiles.md), comment behavior is documented in [`docs/comments.md`](./docs/comments.md), reaction semantics are documented in [`docs/reactions.md`](./docs/reactions.md), and the distinction between a CID and paid persistence is documented in [`docs/media.md`](./docs/media.md).
