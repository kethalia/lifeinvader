...
The profile reader recognizes that deferred-corruption failure explicitly. It closes the partial projection and runs the profile scope's indexed clear before offering another retry, preventing an invalid older page outside the recent preview from failing forever. Canonical keys are removed by one range request; only malformed out-of-range keys consume the bounded cleanup budget.
...
The transaction either commits all five effects or none. A reorg apply first uses the scope index's native count operation to reject orphaned records without loading them into JavaScript. It then walks backward from the authenticated newest record through only the changed suffix. Adjacent cumulative digests and ordinals authenticate that suffix against the retained prefix, and replacement logs extend the retained summary. At most 5,000 removed records are visited; callers may select a lower cap, but never a higher one. Total retained history does not consume that budget, so a one-event reorg remains repairable after 5,001 or more cached events. A larger changed suffix aborts atomically and can be recovered by clearing the disposable scope.

Full resets and explicit clears delete every canonical fixed-width position with one asynchronous IndexedDB key-range request, regardless of scope size. A bounded scope-index cleanup then removes malformed primary-key types that fall outside that canonical range; exceeding the 5,000-record corruption-cleanup ceiling aborts atomically. The generation and revision prevent an ABA race when a reorg returns the cursor fields to an earlier value, so a stale tab cannot overwrite newer canonical history.
