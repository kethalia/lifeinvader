...
The transaction either commits all five effects or none. Reorg rollback sequentially seeks to `rollbackTo` in both the canonical primary position and identity index. It traverses only the affected suffixes newest-first, so corruption in either key cannot hide an orphaned event from the other path. Full resets and explicit clears continue to traverse the separate scope index so malformed primary-key types cannot escape cleanup. The generation and revision prevent an ABA race when a reorg returns the cursor fields to an earlier value, so a stale tab cannot overwrite newer canonical history.
