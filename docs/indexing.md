...
The search is bracketed by selected-chain checks and two reads of one anchored head. Boundary code is re-read before acceptance. The follow stream carries that head fingerprint through its bounded log scan and reauthenticates it before applying the result to IndexedDB. If it changed, the scan result is discarded and boundary discovery is retried once; repeated movement fails without applying either result. This provides the same portable numeric-block compatibility as the event scanner while detecting a reorganization that changes the anchored ancestry. A settled provider rejection on a historical code request is a safe optimization miss: the follow stream starts at genesis instead. A local deadline is propagated without starting a log scan because the underlying wallet request may still be pending. Invalid code, a non-monotonic conflict, malformed quantities or blocks, cancellation, and context changes are not downgraded to a fallback.
