...
The start block and finality depth are explicit cursor inputs. Post, comment, reaction, direct-message, follow, profile, and group-directory streams now obtain that input from a separately verified, bounded protocol-code discovery; group-membership and group-message streams continue to use block zero while the optimization is integrated incrementally. The scanner never silently requests `0x0` through `latest` in one call. Changing the discovered boundary or confirmation policy selects a fresh cursor and cache rather than reinterpreting already accepted ranges.
