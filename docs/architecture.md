...
The global post feed and the independent global comment stream are chain-derived and allow one bounded range per invocation. Neither performs a full-history catch-up loop during normal page load, and the comment stream prevents visible posts from causing an RPC request fan-out. Its recent page is only a preview; complete per-post comment views require an authenticated, bounded local projection. Other unfinished screens may contain fixture content only when fixtures are visibly identified and isolated from chain-derived models.
