...
Posts may reference an IPFS root CID. The client validates a deliberately narrow CID profile and commits canonical CIDv1 bytes. Its optional retrieval boundary has no default provider: users configure a fixed-origin HTTPS gateway URL template and explicitly load individual raw attachments, which are byte-bounded, signature-gated, and SHA-256 verified against the CID before rendering from a temporary blob URL. DAG roots are not fetched without trustless traversal, and the client still does not upload content. The core contract does not collect a media fee that it cannot turn into storage. Future optional storage adapters should route user funds directly to storage providers or proof-linked payment contracts.
