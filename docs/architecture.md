...
Injected wallets are discovered through EIP-6963 and submit writes through their EIP-1193 transport. The client compares exact code hashes before enabling deployment or publishing and repeats the protocol check immediately before every write. The [confirmed post feed](./feed.md) falls back to the connected wallet for reads, or accepts an explicit in-memory HTTP endpoint only after matching its chain and confirmed history against that wallet. The application ships no mandatory vendor endpoint or API key.
...
The global post feed and the independent global comment stream are chain-derived and allow one bounded range per invocation. Neither performs a full-history catch-up loop during normal page load, and the comment stream prevents visible posts from causing an RPC request fan-out. Its recent page is only a preview. Once both streams are caught up, the client authenticates the rendered post-event scope against one fresh bounded feed range before opening a comment projection. That projection scans one bounded cache page per explicit action, validates every global comment, and retains exact histories only for those authenticated visible posts. No result is exposed before the frozen page-lifetime anchor, canonical provider checkpoint, and bracketed cache baseline are reauthenticated, and completed threads remain bounded when read. The direct-message stream likewise allows one bounded range per invocation, but binds its filter and disposable cache scope to one exact symmetric address pair; it never scans the global message family to render a selected conversation. Group-message reads bind the indexed group topic and local cache to one explicit group ID, withhold partial history, and cancel stale work when the selected chain, provider, or group changes. Follow reads similarly bind each cache to one normalized account and either the incoming or outgoing indexed topic, never a global relationship scan; caught-up streams issue authenticated anchors, and a direction-bound runner feeds only exact-scope complete-block cache pages into a deterministic projection before reauthenticating the frozen cache and provider checkpoint. Its React read model exposes one explicit step at a time and drops state whenever the account, direction, chain, or provider changes. Other unfinished screens may contain fixture content only when fixtures are visibly identified and isolated from chain-derived models.

## Media

Posts may reference an IPFS root CID. The client validates a deliberately narrow CID profile and commits canonical CIDv1 bytes. Its optional retrieval boundary has no default provider: users configure a fixed-origin HTTPS gateway URL template and explicitly load individual attachments, which are byte-bounded and signature-gated before rendering from a temporary blob URL. Raw blocks are SHA-256 verified against their CID; larger `dag-pb` files produced by Lifeinvader's deterministic preparation flow are reimported locally and must reproduce their on-chain UnixFS root. Structured DAGs are not fetched without a separate trustless traversal, and the client still does not upload content. The core contract does not collect a media fee that it cannot turn into storage. Future optional storage adapters should route user funds directly to storage providers or proof-linked payment contracts.
