...
The global post feed and the independent global comment stream are chain-derived and allow one bounded range per invocation. Neither performs a full-history catch-up loop during normal page load, and the comment stream prevents visible posts from causing an RPC request fan-out. Its recent page is only a preview. Once caught up, an authenticated local projection scans one bounded cache page per explicit action, validates every global comment, and retains exact histories only for requested visible posts. No result is exposed before the frozen page-lifetime anchor, canonical provider checkpoint, and bracketed cache baseline are reauthenticated, and completed threads remain bounded when read. Other unfinished screens may contain fixture content only when fixtures are visibly identified and isolated from chain-derived models.
