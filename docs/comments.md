...
A completed runner binds that digest to the authenticated comment-cache baseline and can later accept the resulting snapshot, baseline, and cache-issued proof only for the exact same tracked post identifiers. It verifies the saved global count, tail, confirmation, digest, proof, cache generation, and append-only cursor boundary before scanning only newer cache records. The replacement result is rebound only after the new baseline and wallet context are authenticated again.

The read model persists that tuple in a disposable IndexedDB acceleration cache scoped by chain and the canonical visible-post identities `(postId, blockNumber, blockHash, logIndex)`. It does not read saved progress until both the comment stream and the complete visible post feed are caught up and reauthenticated. The store retains the full canonical scope beside its digest-addressed key, and the runner remains the authority that deeply validates and authenticates every untrusted tuple. A rejected or unreadable tuple is removed and bypassed while a fresh projection rebuilds; a save failure never hides a completed comment history.

Deferred corruption detected during a projection triggers a bounded reset of only the comment event-cache scope at that run's verified start block and discards the matching resume tuple. If bounded cleanup succeeds, the user can explicitly rebuild from confirmed events without retrying the rejected tuple. If cleanup itself fails, the client stops offering a futile retry and directs the user to clear the site's browser data. Completed runs release their cache connection while retaining the immutable projection needed for bounded comment pagination.
