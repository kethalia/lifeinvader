...
The projection core can export and defensively restore a schema-versioned snapshot. It preserves the protocol-wide comment count and tail while retaining only comments in the exact tracked post scope. Restoration normalizes every persisted field, rejects inconsistent event, block, transaction, confirmation, and identifier metadata, and produces one canonical digest independent of input order or hex casing. During a rebuild, incremental identity indexes keep each accepted projection page proportional to that page instead of rescanning the retained thread. That digest is intended for a later cache-derived-state binding; it is not authentication by itself. The current runner and read model still perform a fresh in-memory rebuild, while authenticated resume wiring and append-only cache deltas remain separate slices.
