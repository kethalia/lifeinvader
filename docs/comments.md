...
An anchored projection run derives exact histories for one to fifty requested post identifiers without making another RPC request. Each explicit `advance()` scans at most one chronological IndexedDB page. A nominal page contains at most 200 logs; the cache may extend it through the rest of one boundary block, under the existing 5,199-log hard cap. Every global comment is decoded and its protocol-wide, gapless comment identifier is checked even when its post is not requested. Only comments for the requested posts are retained, in canonical oldest-first order.

Partial results are never published. A cache append, rollback, reset, corrupt event, invalid continuation, identifier gap, overlapping call, or closed run fails the projection and discards its derived comments. After the scan completes, a separate bounded step reauthenticates the exact cache generation, revision, cursor, integrity proof, and edge records before binding the result to the stream's confirmed safe-head checkpoint. Completed histories are therefore exact as of that displayed block. The current implementation intentionally performs a fresh in-memory rebuild and is not wired into post cards yet; persistence, append-only deltas, and rendering remain separate slices.
