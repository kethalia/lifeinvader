...
One feed synchronization invocation permits exactly one bounded indexer range. Connecting a wallet performs one invocation. After a post receipt arrives, a separate confirmation monitor reads chain ID and head height every twelve seconds, for at most 30 minutes and 240 attempts. At the target depth it verifies the transaction's current receipt, expected `PostPublished` event, canonical block hash, and a fresh head-height reading; a transaction re-included at a later height waits for twelve blocks from that new inclusion. It triggers one feed invocation only after that canonical evidence is safe. If history remains, the interface exposes a **Load next block range** button; it does not automatically loop toward the head on page load.
