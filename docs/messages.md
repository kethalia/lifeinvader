...
Only logs at least twelve blocks behind the sampled head are accepted. The stream rechecks its final checkpoint, head, and wallet chain after cache work, refuses to claim catch-up if the safe head advances, and lets the shared indexer roll back reorganized checkpoints. Fresh logs are strictly decoded before commit. A malformed cached page clears only that conversation's scope through the indexed reset path and rebuilds it from chain history; malformed primary keys outside the canonical range retain a bounded cleanup ceiling.
