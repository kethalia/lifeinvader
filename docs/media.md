...
Changing wallet context while the Filecoin Pay request is still open immediately turns that no-hash attempt into the same ambiguous state, because aborting an EIP-1193 signal cannot close a wallet prompt or prove that nothing was broadcast. The interface asks the user to close any outstanding prompt and check wallet activity before dismissing the lock. Dismissal invalidates the abandoned operation so a late provider result cannot overwrite a newer attempt.
