...
No funding or upload action is wired into the interface yet. Passing the preflight and reading a quote mean only that the on-chain dependency graph and current cost state were readable; they do not mean the CAR was uploaded, pinned, funded, approved, or committed. The prepared CAR and account-bound quote are the inputs to the next adapter stages. Local integration coverage uses a pinned Anvil fork of the relevant deployed contracts; ordinary CID publication continues to use a separate local Anvil chain.
