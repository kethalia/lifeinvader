...
The provider receives the CAR over credential-free HTTPS and computes its Filecoin PieceCID. Because `CreateDataSet` exposes a payee but no provider ID, this boundary accepts only registrations whose payee is the same address as the selected service account. Immediately before the first signature, the client rereads the complete one-copy quote and requires every reviewed economic and readiness field to remain identical. MetaMask then receives exactly two EIP-712 requests: `CreateDataSet`, whose payee and metadata are fixed to that account, `source=lifeinvader`, and `withIPFSIndexing`; and `AddPieces`, which binds that same client data-set ID to one PieceCID and the prepared IPFS root in `ipfsRootCID`. The adapter rejects any changed signer, chain, FWSS domain, provider, data-set metadata, PieceCID, root CID, extra typed-data field, extra signature, arbitrary contract read, or transaction request, and cryptographically recovers the selected account from each returned signature. The browser wallet never submits the storage transaction: after receiving the signed authorization, the storage provider submits it and may replace it while pending.
Success requires a canonical receipt from the final provider-reported hash with exactly one `DataSetCreated` and one `PieceAdded` event from the pinned FWSS contract. Those events must identify the selected provider, service account, registered payee, and payer, contain no CDN rails, and reproduce the exact data-set metadata, PieceCID, and `ipfsRootCID`. The returned provider URL addresses the stored CAR piece; it is not an assertion that an ordinary IPFS gateway can already retrieve the media. `withIPFSIndexing` requests provider indexing and IPNI announcement, which can complete later or fail independently. The client therefore reports “IPFS indexing requested,” never “pinned” or “indexed.”
Once the CAR has been stored and its PieceCID is known, the primitive emits a self-contained recovery checkpoint. If the provider accepts the signed commit but disconnects before returning a hash, the outcome is explicitly ambiguous because retrying could create a second paid agreement. If a hash was reported, the checkpoint can authenticate its receipt later and derive the data-set and piece IDs directly from the two events. Storage completion remains separate from publishing the media CID in a Lifeinvader post, so either half can succeed while the other does not.
