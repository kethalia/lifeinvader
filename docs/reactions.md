...
If submission may have reached the wallet but no hash comes back, every transaction-producing console remains locked until the user checks wallet activity and acknowledges the ambiguity. If a hash returns but receipt verification becomes unavailable, the shared lock remains until the user checks or explicitly dismisses that hash. This avoids casually duplicating a repost whose status is unknown or opening competing wallet writes. Recovery records retain and display their original account, chain, and provider context. Switching that context while a wallet prompt is still open converts the attempt to a dismissible hashless ambiguity; dismissing it invalidates late submission and completion callbacks. Pending operations with a known hash remain attached to their original context for receipt recovery. The publish console keeps those records independently, so a context change cannot erase an unresolved post or deployment.
...
Before creating fresh cursors, one shared bounded protocol-history discovery finds the earliest block that can contain exact Lifeinvader v1 code. Both filters start at that same authenticated boundary. Only a recognized unavailable or pruned archival-state rejection falls back to genesis; rate limits, transport failures, timeouts, malformed history, code conflicts, cancellation, and context changes fail closed. If deployment is newer than the confirmed head, both streams report the pending boundary and make no log request until a later explicit check.
...
A completed runner now emits that snapshot together with both exact cache baselines and separate keyed derived-state bindings. A later run accepts the tuple only when the snapshot digest matches both bindings, the two baselines share one confirmed boundary, their event tails match the projection's independent progress, their checkpoints remain in both current canonical windows, and each keyed proof authenticates against its own cache scope. It then supplies each baseline to the bounded cache scanner, so unchanged history produces empty pages and newer complete-block pages are applied as deltas. Counts remain unavailable until both resulting baselines are authenticated together and the provider checkpoint is bracketed again.
...
Summaries, the canonical projection snapshot, resume tuple, and completed baselines throw until both streams finish. Final authentication proves both cache snapshots in one read-only IndexedDB transaction, checks the selected chain, shared safe-head checkpoint, and non-regressing issuing head, binds the derived digest separately to each scope, then proves both cache snapshots again before publication. This brackets the provider reads against a concurrent rollback, reset, append, or corruption. Once complete, a summary is exact from the anchor's recorded start block through its safe head.
...
Once the two cache cursors produce a shared projection anchor, the read model loads at most one chain-scoped resume tuple from a separate IndexedDB database. The tuple is never trusted as chain truth: the runner validates and authenticates it against both current event-cache scopes before reuse. An unreadable or rejected tuple is removed and the projection starts fresh. If local storage is unavailable, exact totals still publish after reconstruction, but the UI warns that the next check may repeat that local work.

Each **Process next local reaction page** action invokes exactly one cache scan or final bounded authentication step. A resumed run scans only complete-block deltas after the authenticated baselines; an unchanged history therefore authenticates empty delta pages instead of replaying all retained events. Counts and current-account like state remain absent from every post card until the run completes and the new tuple has either been saved or explicitly reported unsaved. The displayed totals are labeled with their exact start and confirmed-through blocks.

A provider or chain change aborts a network read or final authentication, closes an open local run, clears published totals, and ignores late results from the old context. Deferred projection corruption triggers one bounded clear of each reaction cache scope and discards the resume tuple before offering an explicit rebuild; if that capped cleanup fails, the UI asks the user to clear site data instead of presenting a futile retry.
