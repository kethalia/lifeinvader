...
The feed performs no reaction RPC work merely because a wallet connected. **Load reaction counts** explicitly runs one synchronization invocation: at most one bounded range for likes and one for reposts. If either cursor still has confirmed history remaining, the UI reports both positions and requires another **Load next reaction range** action.
