...
The stream API exposes up to 200 recent validated signals from each cache plus independent progress. It intentionally does not expose counts: a recent page is not complete history. The event cache now provides bounded, hash-chain-verified chronological pages and canonical completed-scan baselines for a later reorg-safe reducer; no aggregate reducer is implemented yet. A future total may be called complete only after that reducer consumes the entire cache snapshot and the corresponding chain cursor reaches its confirmed head.
